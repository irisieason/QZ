    ...
    <div className={chartClasses} role="img" aria-label={`${chartTitle} chart showing maintenance, errors, and offline status over time`}>
      ...
      <div className="status-history-chart__chart">
        ...
        <div className="status-history-chart__canvas">
          {/* 垂直网格线 */}
          <div className="status-history-chart__grid-lines-vertical">
            {xAxisLabels.map((_, index) => {
              const isLeftAxis = index === 0; // 只有左侧是Y轴
              return (
                <div
                  key={index}
                  className={`status-history-chart__grid-line-vertical ${
                    isLeftAxis ? 'status-history-chart__grid-line--bold' : ''
                  }`}
                >
                  {/* X轴刻度标记 */}
                  <div className="status-history-chart__x-tick-mark" />
                </div>
              );
            })}
          </div>
          ...
          <div className="status-history-chart__grid-lines-horizontal">
            {yAxisTicks.map((tick) => (
              <div key={tick} className="status-history-chart__grid-line-horizontal">
                ...
                <div className="status-history-chart__grid-line-horizontal-line" />
              </div>
            ))}
          </div>

          {/* 底部X轴线 */}
          <div className="status-history-chart__x-axis-line" />
          ...
          {/* 数据点 - 使用 HTML 元素 */}
          {Array.isArray(dataSeries) && dataSeries.map((series) => (
            <React.Fragment key={`points-${series.id}`}>
              {series.data.map((point, index) => {
                const svgPoint = dataToSvg(point, maxTimestamp);
                const leftPercent = (svgPoint.x / chartWidth) * 100;
                const topPercent = (svgPoint.y / chartHeight) * 100;
                return (
                  <div
                    key={`${series.id}-${index}`}
      ...
      <div className="status-history-chart__x-axis">
        {xAxisLabels.map((label, index) => {
          const position = (index / (xAxisLabels.length - 1)) * 100;
          return (
            <div
              key={label}
              className="status-history-chart__x-tick"
              style={{ left: `${position}%` }}
            >
              {label}
            </div>
